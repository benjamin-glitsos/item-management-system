...
object Main {
    ...
    def upsertRecord(record: RecordEdit) = {
        sql"""
    ...
    def insertUser(record_id: Int, person: PersonEdit, user: UserEdit) = {
        sql"""
        WITH new_person AS (
            INSERT INTO people (
              record_id
    ...
    def updateUser(record_id: Int, person: PersonEdit, user: UserEdit) = {
        sql"""
    ...
    def upsert(
        ...
        for {
          ...
          _ <- if (r.updated_by.isEmpty) {
                  insertUser(r.id, person, user)
              } else {
                  updateUser(r.id, person, user)
              }
        } yield ()
    }
