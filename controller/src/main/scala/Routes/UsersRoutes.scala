...
// TODO: create a trigger on the users table (not the users with meta view, make a new sql file for this as well) for encrypting the passwords using pgcrypto postgres extension. Then that same extension will be used to decrypt passwords. And you will need https eventually. Use letsencrypt inside the docker container for https?
